...
export default function BrutalistRecognizePage() {
  ...
  const [qwenFoodName, setQwenFoodName] = useState<string | null>(null);
  const [capturedImageData, setCapturedImageData] = useState<string | null>(null);
  const [qwenNutrition, setQwenNutrition] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const storedAnalysisResult = localStorage.getItem("qwenAnalysisResult");
    if (storedAnalysisResult) {
      setQwenFoodName(storedAnalysisResult);
      localStorage.removeItem("qwenAnalysisResult"); // Clean up localStorage
    }

    const storedImageData = localStorage.getItem("capturedImage");
    if (storedImageData) {
      setCapturedImageData(storedImageData);
      localStorage.removeItem("capturedImage"); // Clean up localStorage
    }

    return () => clearTimeout(timer)
  }, [])

  // Function to parse Qwen's nutrition string response
  const parseQwenNutrition = (nutritionString: string) => {
    const data: any = {};
    const pairs = nutritionString.split(', ').map(s => s.trim());
  ...
  useEffect(() => {
    if (qwenFoodName) {
      const fetchQwenNutrition = async () => {
        try {
          const response = await fetch('/api/nutrition-qwen', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({ foodName: qwenFoodName }),
          });

          if (response.ok) {
            const data = await response.json();
            if (data.nutritionData) {
              setQwenNutrition(parseQwenNutrition(data.nutritionData));
            }
          } else {
            console.error("Failed to fetch Qwen nutrition:", response.status, response.statusText);
          }
        } catch (error) {
          console.error("Error fetching Qwen nutrition:", error);
        }
      };
      fetchQwenNutrition();
    }
  ...
  const handleLogFood = () => {
    ...
    const currentNutrition = qwenNutrition || recognizedFood.baseNutrition;
    const loggedCalories = calculateNutrition(currentNutrition.calories);
    const loggedProtein = calculateNutrition(currentNutrition.protein);
    const loggedCarbs = calculateNutrition(currentNutrition.carbs);
    const loggedFat = calculateNutrition(currentNutrition.fat);

    const newLogEntry = {
      name: qwenFoodName || recognizedFood.name,
    ...
    let existingLogs = [];
    try {
      const storedLogs = localStorage.getItem("loggedFoodNutrition");
      if (storedLogs) {
        existingLogs = JSON.parse(storedLogs);
      }
    } catch (e) {
      console.error("Error parsing existing logged food nutrition from localStorage:", e);
    }

    existingLogs.push(newLogEntry);
    localStorage.setItem("loggedFoodNutrition", JSON.stringify(existingLogs));
  }
      ...
      <div className="container mx-auto px-4 py-6 space-y-6">
        ...
        <BrutalistCard>
          <BrutalistCardContent className="p-0">
            <div className="aspect-video overflow-hidden">
              {capturedImageData ? (
                <img src={capturedImageData} alt="Captured Food" className="w-full h-full object-cover" />
              ) : (
        ...
        <BrutalistCard>
          <BrutalistCardHeader>
            <div className="flex items-center justify-between">
              <div>
                <BrutalistCardTitle className="text-2xl">{qwenFoodName || recognizedFood.name}</BrutalistCardTitle>
          ...
          <BrutalistCardContent>
            <div className="nutrisnap-grid-2 gap-4">
              <div className="bg-red-500 text-white p-6 nutrisnap-border text-center">
                <div className="text-4xl font-black mb-2">
                  {calculateNutrition(qwenNutrition?.calories ?? recognizedFood.baseNutrition.calories)}
                </div>
              ...
              <div className="bg-blue-500 text-white p-6 nutrisnap-border text-center">
                <div className="text-4xl font-black mb-2">
                  {calculateNutrition(qwenNutrition?.protein ?? recognizedFood.baseNutrition.protein)}G
                </div>
              ...
              <div className="bg-green-500 text-white p-6 nutrisnap-border text-center">
                <div className="text-4xl font-black mb-2">
                  {calculateNutrition(qwenNutrition?.carbs ?? recognizedFood.baseNutrition.carbs)}G
                </div>
              ...
              <div className="bg-purple-500 text-white p-6 nutrisnap-border text-center">
                <div className="text-4xl font-black mb-2">{calculateNutrition(qwenNutrition?.fat ?? recognizedFood.baseNutrition.fat)}G</div>
            ...
            <div className="mt-6 nutrisnap-grid-2 gap-4 nutrisnap-subtitle text-sm">
              <div className="flex justify-between p-3 bg-gray-100 nutrisnap-border-thin">
                <span>FIBER</span>
                <span className="font-black">{calculateNutrition(qwenNutrition?.fiber ?? recognizedFood.baseNutrition.fiber)}G</span>
              </div>
              <div className="flex justify-between p-3 bg-gray-100 nutrisnap-border-thin">
                <span>SUGAR</span>
                <span className="font-black">{calculateNutrition(qwenNutrition?.sugar ?? recognizedFood.baseNutrition.sugar)}G</span>
              </div>
            </div>
          </BrutalistCardContent>
        </BrutalistCard>
